...
- Pixel-tuned against the supplied ideal/current iPhone screenshots.
- Removed the oversized spacer above the fixed header.
- Settings cog moved back into the black title header.
- Gray row now contains only the order number.
- “Traditions at Scott - Reusepass”, “Preparing Your Order”, and “Return containers within 3 days” are forced onto one line at modern iPhone widths.
- QR section, typography, buttons, and details screen were scaled down to better match the reference proportions.
- Text selection, image dragging, and image-specific context/long-press menus are disabled.

- Final micro-adjustment: black title header reduced slightly in height.
- Main pickup time now pads the hour to two digits (example: 03:18PM) while still using the user's current device time.
- Progress dot now sits at the endpoint of the green progress fill, visually connected to the green line.
